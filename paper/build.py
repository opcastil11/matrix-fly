...
for c, r in runs.items():
    ...
    r["feed_1k"] = [float(r["feeding"][i:i + 1000].mean()) for i in range(0, len(r["feeding"]) - 999, 1000)]; r["feed_min"] = min(r["feed_1k"]); r["energy_min"] = float(r["energy"].min())
    r["gain_1k"] = {ch: [float(r[f"gain_{ch}"][i:i + 1000].mean()) for i in range(0, len(r["feeding"]) - 999, 1000)] for ch in CHANNELS}
...
def sec_results():
    ...
    verdict = (f"Under the calibrated metabolism only <b>{', '.join(dead)}</b> died." if dead else
               f"Nobody died. A's feeding fell below the cost of living for about 2,000 windows in the middle of the run ({' → '.join(f3(x) for x in A['feed_1k'])} per 1,000 windows; cost {f3(cost)}) and its energy dropped to {f2(A['energy_min'])}, the lowest of the four (B {f2(runs['B']['energy_min']) if 'B' in runs else '–'}, C {f2(runs['C']['energy_min'])}, D {f2(runs['D']['energy_min'])}) — then the sugar gate drifted back open ({' → '.join(f2(x) for x in A['gain_1k']['sugar'])}) and feeding recovered to {f3(A['feed_last'])}. The brain shut the door on its own footsteps for good, but not on the sugar: 30% of what arrives on that channel is other brains' traffic the echo head cannot predict, and surprise reopens the gate. In this world the fly does not starve; it goes quiet on the one channel that was purely itself.")
    return f"""
...
<p><b>The door closes.</b> Only A moves its gates (Fig. 3). Touch, the channel that carries its own footsteps back and nothing else, closes all the way: gain {f2(A['gains']['touch'])} at the end of the run. Sugar, which arrives both as the echo of its own feeding and as the ring's ordinary traffic, closes part way and hovers ({' → '.join(f2(x) for x in A['gain_1k']['sugar'])} per 1,000 windows, {f2(A['gains']['sugar'])} at the end); wind ({f2(A['gains']['wind'])}) barely moves and smell and sight, which the ring never sends, stay at 1. Over the whole run A let through {pct(A['sugar_in'])} of the sugar that reached it; D, the same loop with the gates frozen, {pct(runs['D']['sugar_in'])}; C {pct(runs['C']['sugar_in'])}.</p>
<p><b>Feeding dips, then recovers.</b> The connectome's feeding score is driven by sugar (Sec. 2.7: {f3(cal['fed'])} in the 15 windows after sugar gets in vs {f3(cal['quiet'])} in silence, measured on C). A's feeding tracks its sugar gate: {f3(A['feed_first'])} over the first 1,000 windows, {f3(A['feed_min'])} at its lowest thousand, {f3(A['feed_last'])} over the last, while D holds at {f3(runs['D']['feed_first'])} → {f3(runs['D']['feed_last'])} and C at {f3(runs['C']['feed_first'])} → {f3(runs['C']['feed_last'])}. {verdict}</p>
...
<h1>We put a fruit fly brain in the Matrix. It found the glitch and shut the door on its own echo.</h1>
<p class="sub">A 165,122-neuron connectome in a closed loop learns that the world is its own echo, closes the sense it can predict all the way to zero, and eats less for as long as the sugar is predictable too — while three control brains never close anything</p>
...
<p>There is no objective anywhere that mentions simulations. It closes the channels it can predict; its food happens to arrive on one of them; the connectome, receiving less sugar, feeds less; the metabolism runs down. Each step is local. And the same locality is why it does not starve here: the sugar channel carries a 30% share of other brains' traffic that the echo head cannot predict, every surprising arrival reopens the gate, and the door on the food never closes all the way. Touch, which carried nothing but its own footsteps, closed to {f2(A['gains']['touch'])} and stayed there. The brain drew the line exactly where the echo was.</p>
